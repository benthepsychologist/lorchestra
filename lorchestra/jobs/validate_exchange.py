"""Exchange validation job.

Validates raw Exchange email payloads against source schema.
...
def _get_validator():
    """Get cached validator for Exchange schema."""
    import sys
...
def job_validate_exchange_source(bq_client, limit: Optional[int] = None, **kwargs):
    """
    Validate Exchange email records in raw_objects.

    Validates raw payloads against iglu:com.microsoft/exchange_email/jsonschema/1-0-2.
    ...
    run_id = f"validate-exchange-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}"

    logger.info(f"Starting Exchange validation job: run_id={run_id}")
    ...
    logger.info("Querying unvalidated Exchange records...")
    ...
    temp_table_id = f"{dataset}._validation_exchange_temp_{int(time.time())}"
    ...
    logger.info(f"Exchange validation complete: {len(passed_records)}/{len(all_records)} passed, duration={duration_seconds:.2f}s")
