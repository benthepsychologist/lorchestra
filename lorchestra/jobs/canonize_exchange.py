"""Exchange canonization job.

Transforms validated raw Exchange payloads to canonical JMAP Lite format.
Only processes records with validation_status = 'pass' that haven't been canonized yet.

Column Standards:
- source_system: "exchange"
- object_type: "email"
- transform_ref: "email/exchange_to_jmap_lite@1.0.0"
...
def _get_transform():
    """Get cached compiled transform for Exchange."""
...
def job_canonize_exchange_jmap(bq_client, limit: Optional[int] = None, **kwargs):
    """
    Canonize Exchange email records to JMAP Lite format.

    Transforms validated payloads using email/exchange_to_jmap_lite@1.0.0.
    ...
    run_id = f"canonize-exchange-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}"

    logger.info(f"Starting Exchange canonization job: run_id={run_id}")
    ...
    logger.info("Querying validated Exchange records not yet canonized...")
    ...
    if success_records:
        ...
        for batch_start in range(0, len(success_records), batch_size):
            ...
            temp_table_id = f"{dataset}._canonical_exchange_temp_{int(time.time())}_{batch_start}"
    ...
    logger.info(f"Exchange canonization complete: {len(success_records)}/{len(results)} canonized, duration={duration_seconds:.2f}s")
